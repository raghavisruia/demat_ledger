...
import { DematAccount, DematLedgerImportLog } from '@/types/DematLedger'
...
const StatementDetails = ({ data }: Props) => {
    ...
    const { call, loading, error } = useFrappePostCall<{ docs: DematLedgerImportLog[] }>('run_doc_method')
    ...
    const onImport = () => {
        ...
        call({
            ...
            toast.success(_("Broker ledger imported."))
            navigate(`/`)
        }).catch(() => {
            toast.error(_("There was an error while importing the broker ledger."))
        })
    ...
    useFrappeEventListener("demat-ledger-import-progress", (event) => {
        setProgress(event.progress)
    })

    const file_name = data.doc.file.split("/").pop() ?? ""

    const { data: dematAccount } = useFrappeGetDoc<DematAccount>("Demat Account", data.doc.demat_account)
            ...
            <div className='flex flex-col gap-4'>
                ...
                <div className='flex items-start gap-4'>
                    <div className='flex flex-col gap-1'>
                        <H2 className='text-lg border-0 p-0'>{_("Ledger Details")}</H2>
                        <Paragraph className='text-p-sm'><span>
                            {_("We've auto-detected the details of the broker ledger file.")}
                        </span><br />
                            <span>
                ...
                {progress > 0 && <div className='flex flex-col gap-2'><Progress value={progress} max={100} size="lg" />
                    <span className='text-sm'>{_("Importing {0}% of transactions", [progress.toString()])}
                    </span>
                </div>}
                ...
                <Table>
                    <TableBody>
                        <TableRow>
                            <TableHead>{_("Demat Account")}</TableHead>
                            <TableCell>
                                <span className="text-sm">{dematAccount?.account_name ?? data.doc.demat_account}</span>
                                {dematAccount?.broker_name && <span className="text-sm text-ink-gray-5"> ({dematAccount.broker_name})</span>}
                            </TableCell>
                        </TableRow>
                        <TableRow>
                            <TableHead>{_("Ledger Account")}</TableHead>
                            <TableCell>
                                <span title={_("GL Account")} className="text-sm">{dematAccount?.account}</span>
                            </TableCell>
                        </TableRow>
                        <TableRow>
                            <TableHead>{_("Ledger File")}</TableHead>
                            <TableCell>
                        ...
                        <TableRow>
                            <TableHead>{_("Closing Balance as of {0}", [formatDate(data.doc.end_date, "Do MMMM YYYY")])}</TableHead>
                            ...
                            <TableHead>
                                <div className='flex items-center gap-2'>
                                    {_("Detected Amount Format")} <Tooltip>
                                        ...
                                        <TooltipContent>
                                            {_("The amount format detected in the ledger file. This is used to parse the debit and credit values from each row.")}
                                        </TooltipContent>
                                    </Tooltip>
                                </div>
                            </TableHead>
                            ...
                            <TableHead>
                                <div className='flex items-center gap-2'>
                                    ...
                                    <Tooltip>
                                        <TooltipTrigger><InfoIcon size={16} /></TooltipTrigger>
                                        <TooltipContent>
                                            {_("The date format detected in the ledger file. This is used to parse the date values.")}
                                        </TooltipContent>
                                    </Tooltip>
                                </div>
                            </TableHead>
                ...
                <div className='flex flex-col gap-4'>
                    <div className='flex flex-col gap-1'>
                        ...
                        {data.final_transactions?.length === 1 ? (
                            <Paragraph className='text-p-sm'>{_("We've found 1 transaction in the ledger file that will be imported into the system. Please review the details below and click the 'Import' button to proceed.")}</Paragraph>
                        ) : (
                            ...
                            <TableBody>
                                {data.final_transactions?.map((transaction, index) => (
                                    <TableRow key={index}>
                                        ...
                                        <TableCell className='max-w-[120px] w-fit overflow-hidden text-ellipsis'>{transaction.voucher_type}</TableCell>
                                        <TableCell className='max-w-[200px] w-fit overflow-hidden text-ellipsis'>{transaction.description}</TableCell>
                                        <TableCell className='text-end font-numeric'>{formatCurrency(transaction.debit, data.currency)}</TableCell>
                                        <TableCell className='text-end font-numeric'>{formatCurrency(transaction.credit, data.currency)}</TableCell>
                                    </TableRow>
                                ))}
                            </TableBody>
                        </Table>
...
const ConflictingTransactions = ({ transactions }: { transactions: GetStatementDetailsResponse["conflicting_transactions"] }) => {
    ...
    return <>
        <Alert theme="red">
            <AlertCircleIcon />
            <AlertTitle>{_("Possible Duplicate Transactions")}</AlertTitle>
            <AlertDescription>
                {transactions.length === 1 ? _("We've found 1 existing transaction in the system within the same date range for this demat account. Are you sure you want to proceed with the import?")
                    : _("We've found {0} existing transactions in the system within the same date range for this demat account. Are you sure you want to proceed with the import?", [transactions.length.toString()])}
                ...
                <div className='py-2'>
                    <Dialog>
                        ...
                        <DialogContent className='min-w-7xl'>
                            <DialogHeader>
                                <DialogTitle>{_("Possible Duplicate Transactions")}</DialogTitle>
                                <DialogDescription>
                                    {_("Existing transactions in the system belonging to the same demat account and date range.")}
                                </DialogDescription>
                            </DialogHeader>
                                    ...
                                    <TableBody>
                                        {transactions.map((transaction) => (
                                            <TableRow key={transaction.name}>
                                                ...
                                                <TableCell className='text-end font-numeric'>{formatCurrency(transaction.debit, transaction.currency)}</TableCell>
                                                <TableCell className='text-end font-numeric'>{formatCurrency(transaction.credit, transaction.currency)}</TableCell>
                                                <TableCell className='text-end'>
                                                    <Tooltip>
                                                        <TooltipTrigger asChild>
                                                            <Button variant='link' isIconButton asChild className='text-ink-gray-5 hover:text-black p-0 h-4'>
                                                                <a href={`/desk/demat-transaction/${transaction.name}`} target='_blank' rel='noopener noreferrer'>
                                                                    <ExternalLinkIcon />
                                                                </a>
                                                            </Button>
                                                        </TooltipTrigger>
